...
module Wallop
  ...
  def self.snapshot_command(channel, resolution=nil)
    resolution ||= '1280x720'
    file = "#{channel}-#{Time.now.to_i}.png"
    [%{#{config['ffmpeg_path']} -r 1 -f mpegts -analyzeduration 2000000 -i #{raw_stream_url_for_channel(channel)} -vcodec png -map 0:0 -an -sn -updatefirst 1 -t 00:00:01 -s #{resolution} app/public/snapshots/#{file} -loglevel quiet}, file]
  end

  def self.snapshot(channel, resolution=nil)
    cmd, file = snapshot_command(channel, resolution)
